...
export default class GameController {
  ...
  async updateGame(@Param("id") id: number, @Body() update: Partial<Game>) {
    ...
    if (update.id && update.id !== game.id)
      throw new BadRequestError("It is not possible to change the game id.");

    if (update.board) {
      if (moves(game.board, update.board) > 1)
        throw new BadRequestError("Only one move per turn is allowed.");
    }

    if (update.color && !validateColor(update.color.toLowerCase()))
      throw new BadRequestError(
        "The requested color is not defined as a valid color."
      );
